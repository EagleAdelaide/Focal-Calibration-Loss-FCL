"""Minimal ResNet implementations without torchvision.

The original scaffold used torchvision.models for ResNet-18/50.
In some environments (including this packaging environment), importing
torchvision can fail due to missing compiled ops (e.g., torchvision::nms).

To keep this repo runnable out-of-the-box, we ship a small standalone
implementation that covers the needs of CIFAR-style experiments.
"""
...
def resnet18(num_classes: int, cifar_stem: bool = False, weights=None):
    # weights is accepted for API-compat but ignored.
